""" data_processing/customer_manager.py  Manager xử lý nghiệp vụ liên quan đến Khách hàng. """
...
class CustomerManager(BaseManager):
    ...
    def add_points(self, customer_id: str, points: int) -> bool:
        """ Cộng điểm tích lũy cho khách hàng. Returns: bool: True nếu thành công. """
    ...
    def redeem_points(self, customer_id: str, points: int) -> bool:
        """ Trừ điểm khi khách hàng đổi ưu đãi. Returns: bool: True nếu thành công, False nếu không đủ điểm. """
        customer = self.get_by_id(customer_id)
        if not customer:
            return False
        try:
