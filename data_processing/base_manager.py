"""
data_processing/base_manager.py
================================
Lớp cơ sở cho các Manager xử lý nghiệp vụ CRUD.

Nguyên lý OOP áp dụng:
- Template Method Pattern: Định nghĩa khung (framework) cho CRUD,
  lớp con chỉ cần implement _from_dict() và _get_entity_id().
- Dependency Injection: Manager nhận FileHandlerFactory thay vì
  tạo handler trực tiếp → dễ test, linh hoạt.
"""
...
class BaseManager:
    """
    Manager cơ sở cung cấp CRUD hoàn chỉnh thông qua file.

    Lớp con cần override:
        - _from_dict(data: dict) → entity object
        - _get_entity_id(entity) → str
    """

    def __init__(self, data_file: str):
        """
        Args:
            data_file (str): Đường dẫn file lưu dữ liệu (json, csv, txt).
        """
    ...
    def add(self, entity) -> bool:
        ...
        Returns:
            bool: True nếu thành công, False nếu ID đã tồn tại.
        """
    ...
    def update(self, updated_entity) -> bool:
        ...
        Returns:
            bool: True nếu tìm thấy và cập nhật thành công.
        """
        entities = self._load_all()
    ...
    def delete(self, entity_id: str) -> bool:
        ...
        Returns:
            bool: True nếu tìm thấy và xóa thành công.
        """
        entities = self._load_all()
    ...
    def search(self, **filters) -> list:
        """
        Tìm kiếm theo nhiều điều kiện (AND logic).

        Args:
            **filters: Tên thuộc tính và giá trị cần lọc.
                       Hỗ trợ tìm kiếm chuỗi không phân biệt hoa/thường.

        Example:
            manager.search(category="Điện tử", channel="online")
        """
        results = []
        for entity in self._load_all():
            d = entity.to_dict()
